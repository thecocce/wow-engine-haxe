...
package fr.seraf.wow.constraint ;
	 ...
	 */
	class WSpringConstraint extends WBaseConstraint {
		...
		*/	
		 ...
		 */			
		private function get_restLength():Float {
			return restLen;
		}
		 ...
		 * @private
		 */	
		private function set_restLength(r:Float):Float {
			restLen = r;
			return restLen;
		}
		...
		public override function resolve():Void {
			...
			//var dx: Float = ( p1.x + p1.vx ) - ( p0.x + p0.vx );
			//var dy: Float = ( p1.y + p1.vy ) - ( p0.y + p0.vy );
			delta = WVectorMath.sub(p1.curr,p2.curr);
			
			//var d1: Float = Math.sqrt( dx * dx + dy * dy );
			deltaLength =  WVectorMath.distance(p1.curr,p2.curr);
			//var d2: Float = stiffness * ( d1 - restLength ) / d1;
			var diff:Float = stiffness * ( deltaLength - restLength ) / deltaLength;
			//dx *= d2;
			//dy *= d2;
